...
class Visualization {
  ...
  _applyCubeMovement(cube) {
    const velocity = randRange(4, 10);
    const height = randRange(-.4, .4);
    TweenMax.to(cube.position, velocity, { x: -2 , y: height, z: -1,
      onComplete: () => { this._handleRemoveCube(cube); },
      ease: Power0.easeNone,
    });
    TweenMax.to(cube.rotation, velocity, { x: randRotation(), y: randRotation(), z: randRotation(),
      ease: Power0.easeNone
    });
  }
