...
class Visualization {
  ...
  search(str) {
    this.stats.hide();
    this.selectedBySearch = byTag.call(this, str);
    this.currentSearchTerm = str;
    SEObjStartPos = SEOBJ_POSITIONS['a1'];
    Animations.search.call(this, this.selectedBySearch);
  }

  clearSearch() {
    this.stats.show();
    this.currentSearchTerm = '';
    SEObjStartPos = SEOBJ_POSITIONS['start'];
    Animations.reset.call(this, this.selectedBySearch);
    this.selectedBySearch = [];
  }
  ...
  _attachMouseHandlers() {
    ...
    document.addEventListener('mousedown', e => {
      const info = e.path.filter( p => p.id && p.id.includes('close') )[0];
      ...
      if (e.shiftKey) {
        ...
        // clicked on info
        const SEObj = this.renderedSEObjs[info.id.replace('close-', '')];
        SEObj.toggleMoving();
  ...
  _renderFromBuffer() {
    window.setTimeout( () => {
      if (this.SEObjBuffer.length > 0) {
        ...
        SEObj.mesh.position.set(...SEObjStartPos);
...
// This should eventually be moved out of Visualization.js
const MIN_RENDERER_HEIGHT = 800;
const SEOBJ_POSITIONS = {
  start: [2, 0, -1],  // These should be Vector3's
  a1: [5, 0, -1],
};
let SEObjStartPos = SEOBJ_POSITIONS['start'];
