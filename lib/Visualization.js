...
class Visualization {
  ...
  load({ questions, questionDetails, selectedAnswers }) {
    debugger
    // items.forEach( data => {
    //   // const cube = this._createCube(data);
    //   const cube = new SEObject(this, data);
  ...
  _renderCube() {
    ...
    this.scene.add( cube );
    this.st.addCube(cube);
    return cube;
  }
  
  _applyCubeMovement(cube) {
    const duration = randRange(8, 14);
    const height = randRange(-.4, .4);
    const depth = randRange(-.4, .4);
    TweenMax.to(cube.position, duration, { x: -2 , y: height, z: depth,
      onComplete: () => { this._handleRemoveCube(cube); },
      ease: Power0.easeNone,
    });
    TweenMax.to(cube.rotation, duration, { x: randRotation(), y: randRotation(), z: randRotation(),
      ease: Power0.easeNone
    });
  }
